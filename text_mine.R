...
term.freq<-subset(tdm.s, tdm.s>=10)
...
wordcloud(tdm.df$word, tdm.df$freq, min.freq = 2, colors=brewer.pal(9, "BuGn"), random.order=F)
dev.off()

tdm2<-removeSparseTerms(tdm, sparse =0.995)
